...
def create_bar_chart(df):
    chart = alt.Chart(df.reset_index()).encode(
        ...
        y=alt.Y('Project Staffing Percentage', axis=alt.Axis(format='d')),
    )
    return chart.mark_bar() + chart.mark_text(align='center', dx=0, baseline='middle', dy= -5, fontSize=12).encode(text=alt.Text('Project Staffing Percentage', format='d'))
...
project_area_names_tz = st.sidebar.slider("Timezone Critical", min_value=1, max_value=5, help='Describe the need for resources to be time zone aligned' ,key='timezone')
...
#st.divider()
project_area_names_expertise = st.sidebar.slider("Skill Level", min_value=1, max_value=5, help='Skillset necessary for successful delivery of the initiative', key='expert')
