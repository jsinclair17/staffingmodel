...
st.divider()
project_area_names_cost = st.slider("Funding Availability", min_value=1, max_value=10, help='Your enagagment funding level has been allocated', key='cost')
...
st.divider()
project_area_names_speed = st.slider("Go To Market Strategy", min_value=1, max_value=10, help='Describe the level of planning to deliver product', key='speed')
...
project_area_names_tz = st.slider("Time Zone Coordination", min_value=1, max_value=10, help='Describe the need for resources to be time zone aligned' ,key='timezone')
if project_area_names_tz <3:
    ...
    st.write('Required')
st.divider()
project_area_names_complexity = st.slider("Team Size", min_value=1, max_value=10, help='Amount of capacity needed to support the initiative' ,key='complex')
...
st.divider()
project_area_names_expertise = st.slider("Expertise", min_value=1, max_value=10, help='Skillset necessary for successful delivery of the initiative', key='expert')
...
st.divider()
project_area_names_laws = st.slider("Laws & Regulation", min_value=1, max_value=10, help='Barriers for using offshore resources for work', key='laws')
...
st.divider()
project_area_names_accessibility = st.slider("Immediate Response Tolerance", min_value=1, max_value=10, help='Communication promptness required', key="accessible")
...
st.divider()
project_area_names_innovate = st.slider("Innovation & Process Maturity", min_value=1, max_value=10, help='Describe the need to be more Strategic or Tactical', key='innovate')
...
customer1 = stdb.ProjectAdd(customer_name, customer_address, project_name, project_type, project_area_names_cost, project_area_names_speed
